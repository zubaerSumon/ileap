...
console.log("Form has errors:", hasErrors);
  ...
  const volunteerSignup = trpc.auth.volunteerSignup.useMutation({
    onSuccess: () => {
    ...
    onError: (error) => {
      ...
      console.error("Error shape:", JSON.stringify(error, null, 2));
  ...
  const onSubmit = async (data: VolunteerSignupForm) => {
    ...
    try {
      setError(null);
      await volunteerSignup.mutate(data);
    } catch {
  ...
  const handleNext = async () => {
    ...
    const isValid = await trigger(fieldsToValidate as (keyof VolunteerSignupForm)[]);
