...
export default function OrganizationOpportunities({ organizationId }: OrganizationOpportunitiesProps) {
  ...
  const { data: opportunitiesData, isLoading, error } = trpc.opportunities.getAllOpportunities.useQuery({
    page: 1,
    limit: 50,
  });
  ...
  const isOrgAdminOrMentor = session?.user?.role === 'admin' || session?.user?.role === 'mentor';
  ...
  if (error) {
    return (
      <div className="text-center">
        <h1 className="text-2xl font-bold text-red-600">Error loading opportunities</h1>
  ...
  const organizationOpportunities = opportunitiesData?.opportunities?.filter(
    (opp) => (opp as unknown as OpportunityData).organization_profile?._id === organizationId
  ) || [];

  // Calculate available spots for each opportunity
  const opportunitiesWithSpots = organizationOpportunities.map((opportunity) => {
  ...
  if (filteredOpportunities.length === 0) {
    return (
      <div className="text-center py-8">
        <h2 className="text-xl font-semibold text-gray-600 mb-2">No Opportunities Available</h2>
    ...
    <section className="w-full relative">
      ...
      <Tabs defaultValue="all" className="w-full mb-6">
        <TabsList className="grid w-full grid-cols-2">
          ...
          >
            Favorites ({favoriteOpportunities?.filter(fav => 
              opportunitiesWithSpots.some(opp => opp._id.toString() === fav.opportunity)
            ).length || 0})
          </TabsTrigger>
        </TabsList>
      </Tabs>
